...
import 'package:tnazul/models/blacklist.dart';
...
class HomeProvider extends ChangeNotifier {
  ...
  Future<List<Ad>> getAdsSearchList() async {
    final response = await _apiProvider
      ...
      "ads_typee": _typee != null ? _typee : '0',
      "fav_user_id": _currentUser == null ? '' : _currentUser.userId
    });
  ...
  String _selectedTypee = '';

  void setSelectedTypee(String selectedTypee) {
    _selectedTypee = selectedTypee;
    notifyListeners();
  }

  String get selectedTypee => _selectedTypee;
  ...
  void setTypee(String typee) {
    _typee= typee;
    notifyListeners();
  }

  String get typee => _typee;



  String _settingFace = '';

  void setSettingFace(String settingFace) {
    _settingFace = checkedValue;
    notifyListeners();
  }

  String get settingFace => _settingFace;
  ...
  // current seller show
  String _currentSellerShow = '';
  void setCurrentSellerShow(String currentSellerShow) {
    _currentSellerShow = currentSellerShow;
    notifyListeners();
  }
  String get currentSellerShow=> _currentSellerShow;
  ...
  Future<String> getUnreadMessage() async {
    final response =
    await _apiProvider.get("https://tanazul.net/api/get_unread_message?user_id=${_currentUser.userId}");
    String messages = '';
    if (response['response'] == '1') {
  ...
  Future<String> getUnreadNotify() async {
    final response =
    await _apiProvider.get("https://tanazul.net/api/get_unread_notify?user_id=${_currentUser.userId}");
    String messages = '';
    if (response['response'] == '1') {
      messages = response['Number'];
    }
  ...
  Future<String> getSocial() async {
    final response =
    await _apiProvider.get("https://tanazul.net/api/social");
    String messages = '';
    if (response['response'] == '1') {
      messages = response['setting_face'];
    }
    return messages;
  }
