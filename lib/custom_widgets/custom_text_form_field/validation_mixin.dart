...
mixin ValidationMixin<T extends StatefulWidget> on State<T> {
  ...
  String validateAdJob(String title) {
    if (title.trim().length == 0) {
      return AppLocalizations.of(context).translate('validate_job');
    }
    return null;
  }

  String validateNull(String title) {
    if (title.trim().length == 0) {
      return AppLocalizations.of(context).translate('validate_null');
    }
    return null;
  }
  ...
  bool checkAddAdValidation(BuildContext context,
    ...
    if (adCountry == null) {
      Commons.showToast(context,
          message: AppLocalizations.of(context).translate('ad_country_validation'), color: Colors.red);
      return false;
    }
    return true;
  }
